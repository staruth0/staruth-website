import React from "react";
import "./hero.css";

const AboutHeroComponent = () => {
  return (
    <div className="about-hero-section">
        <div className="about-hero-section-contentOne">
          <h1>
            At Staruth, we join forces with start-ups, <br /> brands, businesses
            and founders to build <br /> tech products and systems that help
            them <br /> break out in the digital space
          </h1>
        </div>
        <div className="about-hero-section-contentTwo">
            <h1>Our Vision</h1>
            <p>
              Lorem ipsum dolor sit amet consectetur. Quis orci ses neque
              rhoncus. Integer commodo Lorem ipsum dolor sit amet consepsum
              dolor sit amet consectetur. Quis orci sed fesi egestas neque
              rhoncus. Integer commodo Lorem ipsum dolor sit amet consectetur.
            </p>
          </div>
        <div className="about-hero-section-contentThree">
          <h1>
            From strategic design to <br /> web and mobile apps and <br /> all
            things software, we can <br /> help you
          </h1>
        </div>
      </div>
    </div>
  );
};

export default AboutHeroComponent;
